...
export const generateQuestionsFromTextTool = createTool({
  ...
  inputSchema: z.object({
    extractedText: z
      .string()
      .describe("The extracted text to generate questions from"),
    maxQuestions: z
      .number()
      .optional()
      .describe("Maximum number of questions to generate (default: 10)"),
  }),
  ...
  execute: async ({ context, mastra }) => {
    ...
    if (extractedText.length > MAX_TEXT_LENGTH) {
      console.warn(
        "⚠️ Document is very large. Consider using a smaller PDF to avoid token limits."
      );
    ...
    try {
      ...
      const streamResponse = await agent.streamVNext([
        {
          role: "user",
...
${extractedText.substring(0, MAX_TEXT_LENGTH)}`,
      ...
      if (generatedContent.trim().length > 20) {
        ...
        console.log(
          `✅ Question generation successful: ${questions.length} questions generated`
        );
